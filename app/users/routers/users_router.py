...
from app.users.models import User, Account, Platform
...
@router.get("/user_id/{user_id}", response_model=schema.UserResponse, tags=["users"])
async def read_user(user_id: int, db: db_dependency):
    return find_user_by_id(user_id, db)

# get individual group by Name
@router.get("/user_name/{user_name}", response_model=schema.UserResponse, tags=["users"])
async def read_user_by_name(user_name: str, db: db_dependency):
    return find_user_by_name(user_name, db)

# get all groups
@router.get("/", response_model=list[schema.UserResponse], tags=["users"])
async def read_users(db: db_dependency):
    return find_all_users(db)

# create a new group
@router.post("/", response_model=schema.UserResponse, tags=["users"])
...
@router.patch("/{user_id}", response_model=schema.UserResponse, tags=["users"])
...
@router.delete("/user_id/{user_id}", response_model=schema.UserResponse, tags=["users"])
...
@router.delete("/user_name/{user_name}", response_model=schema.UserResponse, tags=["users"])
